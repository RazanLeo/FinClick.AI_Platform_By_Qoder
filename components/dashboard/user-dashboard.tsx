...
import { financialAnalysisWorkflow } from "@/lib/ai/workflow-orchestrator"
...
export function UserDashboard() {
  ...
  const [analysisHistory] = useState<AnalysisHistory[]>([
    {
      ...
      date: "2024-01-15",
      status: "completed",
      analysisTypes: ["التحليل الشامل - 180 نوع تحليل"],
      filesCount: 5,
    },
    {
      id: "2",
      ...
      date: "2024-01-10",
      status: "completed",
      analysisTypes: ["التحليل الأساسي الكلاسيكي - 106 تحليل"],
      filesCount: 3,
    },
    {
      id: "3",
      ...
      date: "2024-01-08",
      status: "processing",
      analysisTypes: ["التحليل المتقدم والمتطور - 53 تحليل"],
      filesCount: 7,
    },
  ])
  ...
  const handleStartAnalysis = async () => {
    if (uploadedFiles.length === 0) {
      toast.error("يرجى رفع الملفات المالية أولاً")
      return
    }

    if (!analysisOptions.companyName || !analysisOptions.sector || !analysisOptions.legalEntity) {
      toast.error("يرجى إكمال جميع الخيارات المطلوبة")
      return
    }
    ...
    try {
      ...
      setCurrentAnalysisStage('تحضير الملفات...')
      
      toast.success("تم بدء التحليل المالي الذكي!")

      // Validate files first
      const validation = await financialAnalysisWorkflow.validateUploadedFiles(uploadedFiles)
      if (!validation.valid) {
        throw new Error(validation.errors.join(', '))
      }

      // Start the comprehensive AI analysis workflow
      const results = await financialAnalysisWorkflow.executeAnalysis(
        uploadedFiles,
        {
          ...
          comparisonLevel: analysisOptions.comparisonLevel || 'saudi_local',
          analysisType: analysisOptions.analysisType || 'comprehensive',
          includeForecasting: true,
          includeRiskAssessment: true,
          includeBenchmarking: true,
          reportFormat: ['pdf', 'word', 'excel', 'powerpoint']
        },
        // Progress callback
      ...
      if (results.success) {
        toast.success(`تم إنجاز التحليل بنجاح! تم تنفيذ ${results.metadata.totalAnalyses} نوع تحليل`)
      } else {
        toast.error("حدث خطأ أثناء التحليل: " + results.error)
      }

    } catch (error: any) {
      console.error('Analysis error:', error)
      toast.error("حدث خطأ في التحليل: " + error.message)
    } finally {
                  ...
                  {isAnalyzing ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-center space-x-2">
                        <Loader2 className="w-6 h-6 animate-spin text-[#B48500]" />
                        <span className="text-[#B48500] font-bold">جاري التحليل...</span>
                      </div>
                      
                      <div className="w-full bg-black border border-[#B48500] rounded-full h-3">
                        <div 
                          className="bg-gradient-to-r from-[#B48500] to-[#FFD700] h-3 rounded-full transition-all duration-300"
                          style={{ width: `${analysisProgress}%` }}
                        ></div>
                      </div>
                      
                      <div className="text-center">
                        <p className="text-[#B48500] text-sm">{analysisProgress}% - {currentAnalysisStage}</p>
                      </div>
                    </div>
                  ) : (
                    <Button
                    ...
                    >
                      ...
                      بدء التحليل المالي الذكي (180 نوع)
                    </Button>
                  )}
